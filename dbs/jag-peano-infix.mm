...
JAG: BINOP is a type, binop is a var of type BINOP, + * are constants of type binop
     binop var can only take the values of + *
     ?? right ??
$)
...
tsucc $a term S s $.
($ JAG: term ( s binop t ) $)
($ JAG: TODO: we need to include parens ( ) as constants since not using RPN $)
...
($ JAG: wff psi logbinop psi $)
wff_logbinop $a wff logbinop psi phi $.

wff_quant $a wff v quant phi $. 
$(
...
ax-2 $a |- ( phi -> ( phi -> chi ) ) -> ( ( phi -> psi ) -> ( phi -> chi ) )
$(
ax-2 $a |- implies
              ...
              implies
                 ...
                 implies phi chi  $.
$)
ax-3 $a |- ( not phi -> not psi ) -> ( psi -> phi )
$(
ax-3 $a |- implies
...
df-an $a |- ( phi /\ psi ) <=> not ( phi -> not psi ) $.
$(
df-an $a |- iff
               and phi psi
...
eq-congr $a |- ( s0 = s1 /\ t0 = t1 ) -> ( ( s0 binpred t0 ) <=> ( s1 binpred t1 ) ) $.
$(
eq-congr $a |- implies
            and = s0 s1 = t0 t1
...
eq-binop $a  |- s0 = s1 /\ t0 = t1 -> s0 binop t0 = s1 binop t1 $.
$(
eq-binop $a |- implies
                  and = s0 s1 = t0 t1
...
${ $d phi x $.
   alpha_hyp1 $e |- phi -> ( psi <=> chi ) $.
   $(
   alpha_hyp1 $e |- implies phi
              iff psi chi $.
   $)
   alpha_1 $a |- phi -> ( ( quant x psi ) <=> ( quant x chi ) ) $.
   $(
...
${ $d phi x $.
   ...
   alpha_hyp2 $e |- phi -> chi $.
   $(
      alpha_hyp2 $e |- implies phi chi $.
   $)
   alpha_2    $a |- phi -> forall x chi $.
...
${ $d phi y $.
   ...
   alpha_3 $a |- implies forall x forall y ( ( x = y ) -> ( phi <=> psi ) ) ( ( quant x phi  ) <=> ( quant y psi ) ) $.
   $(
...
pa_ax6 $a |- ( x * y ) + x = ( x * S y ) $.
$(
   pa_ax6 $a |- = + * x y x
               * x S y $.
$)

pa_ax7 $a |- ( x < y ) <=> exists z ( y = ( x + S z ) ) $.
$(
   pa_ax7 $a |- iff
             < x y
             exists z = y + x S z $.
